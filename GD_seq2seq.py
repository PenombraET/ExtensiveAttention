...
class Net(nn.Module):
    ...
    def forward(self, x, delta_in):
        ...
        A = nn.Softmax(dim=-1)(self.beta * x)
        Y = torch.einsum('nab,nbd->nad', A, x_tok) 
        return Y
...
def train_student_on_data(
    ...
    for t in range(T):
        ...
        data_loss = torch.sum((y_pred - y_train)**2) /D    
    ...
    with torch.no_grad():
        ...
        data_loss_final   = torch.sum((y_pred_final - y_train)**2 / D).item() 
...
if __name__ == "__main__":
    ...
    alpha_list = np.linspace(0.005, 0.5, 64)
    ...
    for lam_cur in lam_list:
        for Delta_cur in Delta_list:
            ...
            sub_dir = os.path.join(base_dir, f"seq2seq_lam_{lam_cur:.4f}_Delta_in{Delta_in:.2f}")
            ...
            for i in range(samples):
                ...
                with torch.no_grad():
                    ...
                    label_err_i = (torch.sum((y_test_student - y_test_teacher) ** 2 )/(D * N_test)).item()  
