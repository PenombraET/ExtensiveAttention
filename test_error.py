...
def ERM_generalization(overlaps, alpha, param_prior, param_output, samples=1000):
    ...
    for i in range(0,samples):
        zstar, z = np.random.multivariate_normal([0,0],[[Q_0 + noise_in/2,m],[m,q]], L).T  
        ...
        label1 = softmax(zstar_f, betastar)
        label2 = softmax(h_f, beta)

        res += np.dot(label1 - label2, label1 - label2)
